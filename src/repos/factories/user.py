...
class TgUserRepo(RepoFactory):
    ...
    async def get_tg_user_by_tg_id(self, user_id: int) -> T.Optional[TgUser]:
        async with self.session() as session:
            stmt = select(TgUser).filter(TgUser.id == user_id)  # noqa
            user = await session.execute(stmt)
            return user.scalar_one_or_none()

    async def create_tg_user(self, user_id: int, username: T.Optional[str]) -> TgUser:
        user_obj = await self.insert_one(id=user_id, username=username)
        return user_obj
