...
class GPTProducer(RabbitMQProducerFactory):
    ...
    async def create_task_generate_result(self, uq_id: int, local_model: bool = False):
        message = Message(
            body=bytes(json.dumps({'uq_id': uq_id}), 'utf-8'),
            content_type='json',
        )
        await self._publish(message, 'gpt_generate_result_use_local_model' if local_model else 'gpt_generate_result')
    ...
    async def create_task_return_simple_result_to_user(self, user_id: int, result: T.List[str]):
        message = Message(
            body=bytes(json.dumps({'user_id': user_id, 'result': result}), 'utf-8'),
            content_type='json',
        )
        await self._publish(message, 'tg_bot_return_simple_result_to_user')
