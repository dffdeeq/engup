...
class ApiHostProducer(RabbitMQProducerFactory):
    ...
    async def create_task_send_to_transcription(self, filenames: T.List[str]):
        message = Message(
            body=bytes(json.dumps({'files': [os.path.join(TEMP_FILES_DIR, f) for f in filenames]}), 'utf-8'),
            content_type='json',
        )
        await self._publish(message, 'apihost_to_transcription')

    async def create_task_update_answers(self, answers_and_filenames: T.List[T.Dict[str, str]]):
        message = Message(
            body=bytes(json.dumps(answers_and_filenames), 'utf-8'),
            content_type='json',
        )
        await self._publish(message, 'apihost_update_answers')
